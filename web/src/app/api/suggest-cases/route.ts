...
export async function POST(request: NextRequest) {
  ...
  // Fall back to env var if no BYOK provided
  const effectiveKey = apiKey || process.env.ANTHROPIC_API_KEY;
  if (!effectiveKey) {
    return NextResponse.json(
      { error: "API key required. Add one in Settings or set ANTHROPIC_API_KEY env var." },
      { status: 400 },
    );
  }
  ...
  try {
    response = await complete(
      { provider: "anthropic", model: "claude-haiku-4-5-20251001", apiKey: effectiveKey },
    ...
    return NextResponse.json(
      { error: `LLM call failed: ${sanitizeError(err instanceof Error ? err.message : String(err))}` },
      { status: 500 },
    );
  }
