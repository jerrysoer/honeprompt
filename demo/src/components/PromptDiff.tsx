...
function computeDiff(original: string, optimized: string): DiffLine[] {
  ...
  // Build LCS table — indices are always within bounds by loop construction
    ...
    <div className="rounded-2xl border border-border bg-surface overflow-hidden">
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <h3 className="font-heading text-lg font-semibold">
          {hasChanges ? "Optimized Prompt" : "Original Prompt"}
        </h3>
        <div className="flex gap-2">
          {hasChanges && (
            <button
              onClick={() => setShowDiff(!showDiff)}
              className="rounded-lg border border-border px-2.5 py-1 text-xs font-medium text-text-muted hover:bg-surface-alt transition-colors"
            >
          ...
          <button
            onClick={handleCopy}
            className="rounded-lg bg-accent px-2.5 py-1 text-xs font-medium text-white hover:bg-accent-hover transition-colors"
          >
            {copied ? "Copied!" : "Copy"}
          </button>
        </div>
